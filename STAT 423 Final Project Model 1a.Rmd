...
gd_t <- data.frame(gdp = 0.01*gd_data$gdp,
                    divorce_rate = log(0.01*gd_data$divorce_rate + 1))
...
plot(gd_t_model)
```
Not even the box-cox method changes our relationship, with the same scatterplot and residual violations. It seems that transformations can't fix the relationship, but for one last look, we will do an anova test, comparing the models to a null model at significance level $\alpha$ = 0.05.
